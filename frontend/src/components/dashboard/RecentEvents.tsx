...
// Strip journald/syslog prefix from raw message
    ...
    <div className="divide-y divide-gray-50">
      {events.map(ev => (
        <div key={ev.id} className="flex items-start gap-3 py-3">
          <span className="text-lg mt-0.5 shrink-0">{typeIcon[ev.event_type] ?? '•'}</span>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5 flex-wrap">
              <span className="text-xs font-medium text-gray-500">{ev.server_name}</span>
              <span className={clsx('text-xs px-1.5 py-0.5 rounded-full font-medium', severityBadge[ev.severity])}>
                {ev.severity}
              </span>
